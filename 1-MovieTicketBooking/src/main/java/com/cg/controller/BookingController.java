...
public class BookingController {
    ...
    public String confirmBooking(@PathVariable Long showId,
        ...
        // Split seat names
        List<String> seatNameList = Arrays.stream(seatIds.split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
                .toList();

        if (seatNameList.isEmpty()) {
            ra.addFlashAttribute("error", "Please select seats!");
        ...
        try {
            ...
            // ⭐ ALWAYS REDIRECT TO PAYMENT PAGE AFTER SUCCESS
            ...
            // ⭐ ABSOLUTELY CORRECT AND CLEAN REDIRECT
            return "redirect:/seats/" + showId;
        }
    }
