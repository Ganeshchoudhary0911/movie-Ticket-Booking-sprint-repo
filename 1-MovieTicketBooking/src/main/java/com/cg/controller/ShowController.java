...
public class ShowController {
	...
	public String showTimings(@PathVariable Long movieId, @RequestParam(required = false) LocalDate date, Model model) {
		...
		List<LocalDate> dates = allShows.stream().map(ShowDto::getShowDate).filter(Objects::nonNull).distinct().sorted()
				.toList();
		...
		List<ShowDto> showsForDate = (selectedDate == null) ? List.of()
				: allShows.stream().filter(s -> selectedDate.equals(s.getShowDate())).toList();
	...
	public String addShow(@Valid @ModelAttribute("show") ShowDto showDto, BindingResult result, Model model) {
		...
		return "redirect:/admin/shows?success=created";
	}
	...
	public String updateShow(@PathVariable Long id, @Valid @ModelAttribute("show") ShowDto showDto,
